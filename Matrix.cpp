...
void Matrix:: setUpMatrix(bool initialArgs[10][10])
{
...
void Matrix::updateMatrix()
{
  for (int i=0;i<this.a;i++)
  {
    for (int j=0;j<this.b;j++)
    {
      ...
      if else (cells[i][j].isAlive()&&(cells[i][j].getNeighborsAlive()>3||cells[i][j].getNeighborsAlive()<2))
...
void Matrix::getNumofAliveN(Cell c)
{
  int x=c.x;
  int y=c.y;
  int neighbors=0;
